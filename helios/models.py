...
class Election(HeliosModel):
  ...
  def issues_before_freeze(self):
    ...
    if self.questions == None or len(self.questions) == 0:
      issues.append(
        {'type': 'questions',
         'action': "adicionar questões as cédulas"}
        )
    ...
    if self.voter_set.count() == 0 and not self.openreg:
      issues.append({
          "type" : "voters",
          "action" : 'insira os eleitores (ou abra a eleição como pública)'
          })
  ...
  def combine_decryptions(self):
    ...
    #contagem de votos com o peso
    self.result = self.encrypted_tally.decrypt_from_factors(decryption_factors, self.public_key)*self.peso
